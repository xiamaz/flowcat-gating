...
kOutput <- "output/gated_single_removeedge"
...
# Uses edge event code from: https://github.com/SofieVG/FloReMi
FilterEdgeEvents <- function(data, dimensions) {

  chNames <- data@parameters@data$name
  names(chNames) <- data@parameters@data$desc

  # Look op the accepted ranges for the dimensions
  meta <- pData(data@parameters)
  rownames(meta) <- meta[,"name"]
  selection <- rep(TRUE,times=dim(data)[1])

  e <- exprs(data)
  # Make selection
  for(dd in dimensions){
    d <- as.character(chNames[dd])
    selection <- selection &
...
ProcessSample <- function(inputPath, outputDir) {
  ...
  inputData <- FilterEdgeEvents(inputData, c("SS INT LIN", "FS INT LIN", "CD45-KrOr"))
  tryCatch({
